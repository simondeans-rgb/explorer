...
// photography on the cover, a pictures spread, a deep-navy numbers page and
// photo-headed continent chapters. Rendered via expo-print and handed to the
// native share sheet. Lazy-imports the native modules like mapPoster so loading
// this module never pulls one in at startup.
//
// Photos are inlined as base64 data URIs before the HTML is built — expo-print
// snapshots the page without waiting for remote images, so embedding is the
// only reliable way to get photography into the PDF.
...
function coverPage(input: AlmanacBookInput, photos: PhotoMap): string {
  const first = (input.firstName || 'Explorer').split(' ')[0];
  const hero = input.heroCode ? photos.get(input.heroCode) : undefined;
  const media = hero
    ? `<img class="fill" src="${hero}"><div class="fill coverscrim"></div>`
...
function picturesPage(input: AlmanacBookInput, photos: PhotoMap): string {
  ...
  const card = (c: { name: string; code: string; src: string }, cls: string) =>
    `<div class="pcard ${cls}"><img class="fill" src="${c.src}"><div class="fill pscrim"></div>
      <div class="pcap">${flagEmoji(c.code)}<span>${esc(c.name)}</span></div></div>`;
  return `<section class="page light">
    <div class="eyebrow coral">CHAPTER ONE</div>
...
function figuresPage(input: AlmanacBookInput): string {
  ...
  return `<section class="page dark">
    <div class="eyebrow coral">CHAPTER TWO</div>
...
function continentPages(input: AlmanacBookInput, photos: PhotoMap): string {
  ...
  return input.continents
    .map((c) => {
      const src = c.coverCode ? photos.get(c.coverCode) : undefined;
      const media = src ? `<img class="fill" src="${src}">` : `<div class="fill covergrad"></div>`;
      return `<section class="page light">
        <div class="eyebrow coral">CHAPTER THREE &nbsp;&middot;&nbsp; THE WORLD</div>
        <div class="kicker">${esc(c.name)}</div>
        <div class="conthero">${media}<div class="fill pscrim"></div>
...
export function buildAlmanacBookHtml(input: AlmanacBookInput, photos: PhotoMap = new Map()): string {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    ...
    .page { width: 720px; min-height: 1018px; box-sizing: border-box; padding: 56px; page-break-after: always; position: relative; overflow: hidden; }
    .page:last-child { page-break-after: auto; }
    .light { background: ${PAPER}; color: #16203A; }
    .dark { background: ${NAVY}; color: #fff; }
    .fill { position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; }

    .cover { padding: 0; height: 1018px; }
    ...
    .plead { height: 300px; margin-bottom: 14px; }
    .pgrid { display: flex; flex-wrap: wrap; gap: 14px; }
    .pgrid .pcard { width: 297px; height: 200px; }
    ...
    .rec { background: #fff; border-radius: 18px; padding: 16px 18px; display: flex; gap: 16px; align-items: center; box-shadow: 0 2px 8px rgba(22,32,58,.06); page-break-inside: avoid; }
