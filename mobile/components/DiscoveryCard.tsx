...
export function DiscoveryCard({ discovery: d, onPress }: { discovery: Discovery; onPress?: () => void }) {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`${d.name}${place ? `, ${place}` : ''} · ${DISCOVERY_CATEGORY_META[d.category].label}${d.verdict ? `, ${VERDICT_META[d.verdict].label}` : ''}`}
      className="bg-white dark:bg-card rounded-3xl flex-row items-start"
      style={{ padding: 14, gap: 12, borderLeftWidth: 3, borderLeftColor: cat, ...SHADOW.card }}
    >
      {d.photo ? (
